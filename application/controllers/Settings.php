...
class Settings extends CI_Controller {
   ...
   function __construct( )
					...
					$config = array(
						...
						if(empty($_FILES['file']['name'])){
				   		...
				   		if($this->upload->do_upload('filebg')){ 
						     ...
						     $insertarray['s_bg_img'] = $uploadData1['file_name'];
					    } else { 
					    	 $successBg=false;
					    	 $msg = $this->upload->display_errors();
					    } 
	...
	public function packagesubscription(){
		...
		$data['packages_subscription'] = $this->package_model->getall_packageSubscription($sCond);
		$data['activePackages'] = $this->package_model->getActive_packages();
		
		$this->template->template_render('package_subscription_setting',$data);
	}
	public function applySubscription($P_id){

		$data['website_setting'] = $this->db->select('*')->from('settings')->get()->result_array();
		$data['packagesDetails'] = $this->package_model->get_packagedetails($P_id);
		$this->template->template_render('package_subscription_payment',$data);
	}
	public function save_subscription_paymeny(){
		
		$paymentMode = $this->input->post('method');
		$ip_id       = $this->input->post('ps_id');

		$aUserData = $this->session->userdata('session_data');
		$website_setting = $this->db->select('*')->from('settings')->get()->result_array();

		$dCurrentDate = date('Y-m-d');
		$packagesDetails = $this->package_model->get_packagedetails($ip_id);

		$AcvtivePackage = $this->package_model->getall_packageSubscription();
		
		$aData['ps_company_name']= $website_setting[0]['s_companyname'];
		$aData['ps_current_package']= $AcvtivePackage[0]['ps_package'];
		$aData['ps_current_package_value']=$AcvtivePackage[0]['ps_package_amount'];
		$aData['ps_current_package_periods_in_days']=$AcvtivePackage[0]['ps_package_period'];
		$aData['ps_package_id']= $packagesDetails[0]['p_id'];
		$aData['ps_package']= $packagesDetails[0]['p_name'];
		$aData['ps_package_currency']= $packagesDetails[0]['p_currency'];
		$aData['ps_package_amount']= $packagesDetails[0]['p_price'];
		$aData['ps_package_period']=$packagesDetails[0]['p_interval'];
		$aData['ps_start_date'] = $dCurrentDate;
		$aData['ps_end_date']=date('Y-m-d', strtotime($dCurrentDate. ' + '.$packagesDetails[0]['p_interval'].' days'));
		$aData['ps_trial_end_date']=date('Y-m-d', strtotime($dCurrentDate. ' + '.$packagesDetails[0]['p_trial_days'].' days'));
		$aData['ps_status']='waiting';
		$aData['ps_date']=$dCurrentDate;
		$aData['ps_created_date']=$dCurrentDate;
		$aData['ps_transcation_id']= '';
		$aData['business_id'] = $aUserData['u_id'];
		$aData['paid_via'] = ($paymentMode == 1 ? "payhere":"offline");
		$aData['ps_createdby'] = ($aUserData['u_id'] == 1 ? "admin":"member");	
		
		$res = $this->db->insert('package_subscription',$aData);
		if($res){
			echo 1;	
		}else{
			echo 0;
		}
		//redirect('settings/packagesubscription');
