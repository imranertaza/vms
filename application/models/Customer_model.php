...
class Customer_model extends CI_Model{
	...
	public function getall_customer_for_dropdown() { 

		return $this->db->select('*')->from('customers')->order_by('c_id','desc')->get()->result_array();

	} 
	public function getall_customer_by_id($iCustId){
		return $this->db->select('*')->from('customers')->where('c_id',$iCustId)->order_by('c_id','desc')->get()->result_array();
	}
